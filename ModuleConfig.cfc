...
*/
    ...
    this.description        = "A simple module to enable use of the multiple custom code editors";
    this.version            = "1.0";
    ...
    function configure(){
        ...
        // Binder Mappings        
        binder.map( "AceEditor@CodeEditor" ).to( "#moduleMapping#.model.AceEditor" );
        binder.map( "CodeMirrorEditor@CodeEditor" ).to( "#moduleMapping#.model.CodeMirrorEditor" );
    ...
    */
    function onLoad(){
        ...
        var AceEditor = controller.getWireBox().getInstance( "AceEditor@CodeEditor" );
        var CodeMirrorEditor = controller.getWireBox().getInstance( "CodeMirrorEditor@CodeEditor" );
        if( AceEditor.getSetting( "active") ) {
            EditorService.registerEditor( controller.getWireBox().getInstance("AceEditor@CodeEditor") );
        }
        if( CodeMirrorEditor.getSetting( "active" ) ) {
            EditorService.registerEditor( controller.getWireBox().getInstance("CodeMirrorEditor@CodeEditor") );
        }
        // Let's add ourselves to the main menu in the Modules section
        var menuService = controller.getWireBox().getInstance("AdminMenuService@cb");
        // Add Menu Contribution
        menuService.addSubMenu(topMenu=menuService.MODULES,name="CodeEditor",label="Custom Code Editor Manager",href="#menuService.buildModuleLink('CodeEditor','home')#");
    }
